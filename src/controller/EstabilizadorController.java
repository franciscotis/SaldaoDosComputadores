...
public class EstabilizadorController implements Initializable //Controller da tela de establizador
{
    ...
    public void removerItem(ActionEvent event) throws IOException, NotBoundException, AlreadyBoundException {
        ...
        LojaInterface loja = (LojaInterface) Naming.lookup("rmi://"+ClientController.getIps()+":5099/loja"); //Acessa o objeto remoto
        boolean res = loja.removerItem("Estabilizador",this.qtd.getValue()); //Chama o método de remover o item no servidor e armazena o resultado
        for(Client cl : ClientController.getInstance().getLojas() ){ //Percorre as lojas
            if(cl.getNome().equals(ClientController.getInstance().getCurrentStore())){
                this.disponibilidade.setText(cl.getQuantidade("Estabilizador")+ " itens no estoque!"); //Modifica a quantidade total do produto
