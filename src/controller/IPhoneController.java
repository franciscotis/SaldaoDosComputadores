...
public class IPhoneController implements Initializable { //Controller do iPhone
    public static final String titulo = "Compra de iPhone";

    @FXML
    private Label disponibilidade;

    @FXML
    private  Spinner<Integer> qtd = new Spinner<Integer>();
    ...
    public void removerItem(ActionEvent event) throws IOException, NotBoundException, AlreadyBoundException {
        ...
        boolean res = loja.removerItem("iPhone",this.qtd.getValue()); //Chama o método de remover o item no servidor e armazena o resultado
        for(Client cl : ClientController.getInstance().getLojas() ){ //Percorre as lojas
            if(cl.getNome().equals(ClientController.getInstance().getCurrentStore())){
                this.disponibilidade.setText(cl.getQuantidade("iPhone")+ " itens no estoque!");  //Modifica a quantidade total do produto
    ...
    public void initialize(URL location, ResourceBundle resources) { //Método "construtor" da tela
        ...
        try {
            ...
            for(Client cl : ClientController.getInstance().getLojas() ){
                if(cl.getNome().equals(ClientController.getInstance().getCurrentStore())){
                    this.disponibilidade.setText(cl.getQuantidade("iPhone")+ " itens no estoque!");
                }
            }
