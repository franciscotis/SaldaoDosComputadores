...
public class TabletController implements Initializable {// Controller do Tablet
    public static final String titulo = "Compra de Tablet";
    ...
    public void removerItem(ActionEvent event) throws IOException, NotBoundException, AlreadyBoundException {
        ...
        boolean res = loja.removerItem("Tablet",this.qtd.getValue()); //Chama o método de remover o item no servidor e armazena o resultado
        for(Client cl : ClientController.getInstance().getLojas() ){ //Percorre as lojas
            if(cl.getNome().equals(ClientController.getInstance().getCurrentStore())){
                this.disponibilidade.setText(cl.getQuantidade("Tablet")+ " itens no estoque!");  //Modifica a quantidade total do produto
    ...
    public void initialize(URL location, ResourceBundle resources) { //método "Construtor" da tela
        ...
        try {
            ...
            for(Client cl : ClientController.getInstance().getLojas() ){
                if(cl.getNome().equals(ClientController.getInstance().getCurrentStore())){
                    this.disponibilidade.setText(cl.getQuantidade("Tablet")+ " itens no estoque!");
                }
            }
